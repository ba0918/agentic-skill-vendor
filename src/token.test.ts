...
import { requireUsableToken, TOKEN_LIMIT } from "./token.ts";
...
test("no refusal repeats the value it refused", () => {
  ...
  for (const raw of [
    `${secret}\nX-Injected: yes`,
    `${secret} ${secret}`,
    `${secret}${"g".repeat(TOKEN_LIMIT)}`,
  ]) {
    const refusal = refusalOf(raw);
    expect(refusal).not.toContain(secret);
    expect(refusal).not.toContain("supersecret");
  }
});

test("the token shapes GitHub hands out are taken as they are", () => {
  for (const token of [
    "ghp_16CharactersOfBase62xxxxxxxxxxxxxxxx",
    "github_pat_11ABCDEFG0abcdefghij_KLMNOPqrstuvwxyz0123456789",
    "gho_16CharactersOfBase62xxxxxxxxxxxxxxxx",
    "v1.1f699f1069f60xxxxxxxxxxxxxxxxxxxxxxxxx",
  ]) {
