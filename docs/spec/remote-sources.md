...
mode の検査は、実際に取得するファイル — 対応表が指す契約ファイルと、その隣の conformance
ツリー — に限って行う。この範囲に通常ファイル(git の mode 100644 / 100755)以外が在れば、
path と mode を挙げて拒否する。黙って落とすと「取得元が持っていない」と「取得元は持つが
取れない形で持っている」が区別できなくなり、在る conformance ツリーが不在として pin される
ためである。conformance ツリーの位置そのものに立つエントリ(リンクやサブプロジェクト)も、
同じ理由でこの範囲に含む。範囲外のエントリは mode に関わらず無視し、取得も digest も pin も
しない。取得元の木の列挙はリポジトリ全体を覆うため、列挙の時点で拒否すると、契約と無関係な
位置にある symlink 1 つで、そのリポジトリが持つ全契約が取得不能になる。読まないファイルは、
その取り違えを起こしようがない。
